...
function openSecureContextHelp() {
  ...
  win.body.innerHTML = `
    ...
    <p><b>2. LAN testing: use the HTTPS port.</b> The dev server
    (<code>python serve.py</code>) serves HTTPS alongside HTTP — just open
    <a href="https://${location.hostname}:8443${location.pathname}${location.search}">
    https://${location.hostname}:8443</a> and accept the
    self-signed-certificate warning once.</p>
...
function openHelp() {
  ...
  win.body.innerHTML = `
    ...
    <code>js/config.js</code> or load one via <code>?character=URL</code>.</p>
...
async function start() {
  ...
  // greeting once the renderer is up
  const greet = () => {
    if (!showroom?.ready) { setTimeout(greet, 400); return; }
    const msg = `Welcome to ${CONFIG.appName}! *boot chime* I am ${CONFIG.guideName}. Browse my creator's projects with ◀ ▶, toss the exhibits around, ask me how they work — or tell me to dance!`;
    chatPrint('bot', msg);
    showroom.character.talk(msg, 'wave');
  };
