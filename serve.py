...
HTTPS_FALLBACK = 8443          # used when the primary HTTPS port is taken
...
def main():
    ...
    if ensure_cert():
        httpsd = None
        port_used = None
        for port in dict.fromkeys([HTTPS_PORT, HTTPS_FALLBACK]):
            try:
                httpsd = http.server.ThreadingHTTPServer(('0.0.0.0', port), Handler)
                port_used = port
                break
            except OSError as err:
                print('HTTPS port %d unavailable (%s)' % (port, err))
        if httpsd:
            ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
            ctx.load_cert_chain(CERT, KEY)
            httpsd.socket = ctx.wrap_socket(httpsd.socket, server_side=True)
            servers.append(httpsd)
            suffix = '' if port_used == 443 else ':%d' % port_used
            print('HTTPS  https://localhost%s' % suffix)
            print('       https://%s%s   <- use this from other devices' % (lan_ip(), suffix))
            print('       (self-signed cert: accept the browser warning once)')
        else:
            print('HTTPS disabled — HTTP still up')
    else:
